            ...
            session.player.getStatus((err, status) => {
                if (!err && status) {
                    ...
                    if (health.reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
                        ...
                        // The device went silent without ever firing the
                        // player/client 'close' or 'error' events that
                        // normally trigger cleanup (e.g. unplugged, dropped
                        // off the network) — tear the session down here so
                        // activeSessions and its dependent maps don't leak.
