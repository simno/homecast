 ...
 * Used by development mode (npm run dev) and the tests
 */
...
class MockChromecast extends EventEmitter {
    constructor(name = 'Mock Chromecast', port = 8009) {
        ...
        this.liveEnded = false; // The broadcast has finished (#EXT-X-ENDLIST)
        ...
        const fetchPlaylist = () => {
            ...
            protocol.get(playlistUrl, (res) => {
                ...
                res.on('end', () => {
                    ...
                    if (!isLive && this.media?.streamType === 'LIVE') this.endBroadcast();
    ...
    play() {
        ...
        this.playbackInterval = setInterval(() => {
            if (this.playerState === 'PLAYING') {
                ...
                if (this.liveEdgeTime !== null && !this.liveEnded) {
                    this.liveEdgeTime += 1; // Live edge moves forward 1 second per second
                }

                // A finished broadcast plays out to its last segment, then stops.
                if (this.liveEnded && this.currentTime >= this.liveEdgeTime) {
                    this.finish();
                    return;
                }
    ...
    // The broadcast is over: the live edge stops moving and becomes the end.
    endBroadcast() {
        if (this.liveEnded || this.liveEdgeTime === null) return;
        console.log(`[MockCast] Broadcast ended at ${this.liveEdgeTime.toFixed(1)}s`);
        this.liveEnded = true;
        this.emit('status', this.getStatus());
    }

    // Played to the end of the media.
    finish() {
        console.log('[MockCast] Playback finished');
        this.playerState = 'IDLE';
        this.idleReason = 'FINISHED';
    ...
    seek(time) {
        console.log(`[MockCast] Seeking to ${time.toFixed(1)}s`);
    ...
    stop() {
        ...
        this.playerState = 'IDLE';
        this.idleReason = 'CANCELLED';
        this.currentTime = 0;
        this.media = null;
        this.liveEdgeTime = null;

        // A stop during the initial buffering must not start playback later.
        clearTimeout(this.startTimeout);
    ...
    getStatus() {
        ...
        if (this.media && this.media.streamType === 'LIVE' && this.liveEdgeTime !== null) {
            status.liveSeekableRange = {
                ...
                isMovingWindow: !this.liveEnded,
                isLiveDone: this.liveEnded
            };
        }
        if (this.playerState === 'IDLE' && this.idleReason) status.idleReason = this.idleReason;
...
class MockCastClient extends EventEmitter {
    constructor(device) {
        ...
        this.volume = { level: 0.5, muted: false };
    }
    ...
    getStatus(callback) {
        callback(null, { volume: this.volume });
    }
    ...
    setVolume(volume, callback) {
        ...
        console.log(`[MockCast] Volume: ${JSON.stringify(this.volume)}`);
        this.emit('status', { volume: this.volume });
        if (callback) callback(null, this.volume);
    }
...
class MockPlayer extends EventEmitter {
    ...
    seek(currentTime, callback) {
        this.mockDevice.seek(currentTime);
