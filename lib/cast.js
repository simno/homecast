...
async function castToDevice(ip, url, proxy, referer, res) {
    ...
    if (isMockDevice) {
        ...
        const mockModule = require('../test/mock-chromecast');
    ...
    // Pre-flight: verify the device is reachable. If not, try Wake-on-LAN.
    try {
        await checkReachable(ip);
        console.log(`[Cast] Device ${ip}:8009 is reachable`);
    } catch (err) {
        console.log(`[Cast] Device ${ip} unreachable (${err.message}), attempting Wake-on-LAN...`);

        const deviceId = devices[ip]?.id;
        const woken = await wakeDevice(ip, deviceId);

        if (woken) {
            // Wait for the device to boot its network stack (TVs can take 10-20s)
            console.log(`[Cast] WOL sent, waiting for ${ip} to wake...`);
            for (let attempt = 1; attempt <= 6; attempt++) {
                await new Promise(r => setTimeout(r, 5000));
                try {
                    await checkReachable(ip, 8009, 3000);
                    console.log(`[Cast] Device ${ip} woke up after ${attempt * 5}s`);
                    break; // Proceed to cast
                } catch {
                    console.log(`[Cast] Still waiting for ${ip} (attempt ${attempt}/6)...`);
                    if (attempt === 6) {
                        if (!res.headersSent) {
                            res.status(502).json({
                                error: `Sent Wake-on-LAN packet but ${ip} did not wake up within 30s. The device may be fully powered off or on a different network.`,
                                troubleshooting: {
                                    serverIp: getLocalIp(),
                                    deviceIp: ip,
                                    check: 'Is the TV plugged in and on the same subnet? WOL only works over wired Ethernet on most TVs (not WiFi).'
                                }
                            });
                        }
                        return;
            ...
            if (!res.headersSent) {
                res.status(502).json({
                    error: `Cannot reach device at ${ip}:8009 — ${err.message}. No MAC address available for Wake-on-LAN.`,
                    troubleshooting: {
                        serverIp: getLocalIp(),
                        deviceIp: ip,
                        check: 'Device may be offline or on a different network. If using Docker on macOS, \'network_mode: host\' does not give direct LAN access — run without Docker.'
                    }
                });
            }
            return;
    ...
    client.connect(ip, () => {
        ...
        launchReceiver(DefaultMediaReceiver, (err, player) => {
            ...
            const isLiveUrl = /type=live|master_dynamic|live\.m3u8|_live\.m3u8/i.test(finalUrl + url);
            const streamType = isLiveUrl ? 'LIVE' : 'BUFFERED';
            ...
            if (isLiveUrl) loadOptions.currentTime = 86400;
