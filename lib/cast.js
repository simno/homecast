...
// Derive Chromecast content type and video type from URL
function getContentType(finalUrl) {
...
async function ensureReachable(ip, res) {
    ...
    if (!woken) {
        if (!res.headersSent) {
            res.status(502).json({
                error: `Cannot reach device at ${ip}:8009 — No MAC address available for Wake-on-LAN.`,
                troubleshooting: {
                    serverIp: getLocalIp(),
                    deviceIp: ip,
...
async function castToDevice(ip, url, proxy, referer, quality, res) {
    console.log(`[Cast] Request received for IP: ${ip}, URL: ${url}, Proxy: ${proxy}, Referer: ${referer}, Quality: ${quality}`);
    ...
    const finalUrl = proxy
        ? `http://${localIp}:${PORT}/proxy?url=${encodeURIComponent(url)}&referer=${encodeURIComponent(referer || '')}&quality=${encodeURIComponent(quality || 'highest')}`
        : url;

    console.log(`[Cast] Final Media URL: ${finalUrl}`);
    const { contentType, videoType } = getContentType(finalUrl);
    ...
    client.connect(ip, () => {
        ...
        launchReceiver(DefaultMediaReceiver, (err, player) => {
            ...
            const isLiveUrl = /type=live|master_dynamic|live\.m3u8|_live\.m3u8|\/api\/channel\/hls\//i.test(finalUrl + url);
            const media = {
                contentId: finalUrl,
                contentType,
                streamType: isLiveUrl ? 'LIVE' : 'BUFFERED'
            };
            ...
            player.load(media, loadOptions, (err, status) => {
                ...
                initializeStreamRecovery(ip, url, referer || '', quality || 'highest');
...
module.exports = { castToDevice, stopCasting, cleanupSessionMaps };
