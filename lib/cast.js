...
const { detectFrameRate, isLiveHlsStream } = require('./extraction');
...
async function castToDevice(ip, url, proxy, referer, quality, res) {
    ...
    const streamType = videoType === 'hls' ? await resolveStreamType(url, finalUrl) : 'BUFFERED';
    ...
    client.connect(ip, () => {
        ...
        launchReceiver(DefaultMediaReceiver, (err, player) => {
            ...
            player.load(media, loadOptions, (err, status) => {
                ...
                initializeStreamRecovery(ip, url, referer || '', quality || 'highest', streamType);
