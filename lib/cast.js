...
// 'seek' (value: seconds to skip, negative for back), 'volume' (value: 0-1)
...
async function controlPlayback(ip, action, value) {
    ...
    switch (action) {
        ...
        case 'seek': {
            ...
            let target = Math.max(0, (status?.currentTime || 0) + value);
            if (Number.isFinite(duration) && duration > 0) target = Math.min(target, Math.max(0, duration - 1));
            await call(cb => player.seek(target, cb));
            break;
        }
        case 'volume':
...
module.exports = { castToDevice, stopCasting, controlPlayback, cleanupSessionMaps, getContentType };
