...
const { validateProxyUrl, safeLookup } = require('./security');
...
async function extractIframesFromScripts(html, pageUrl) {
    ...
    for (const scriptUrl of scriptUrls.slice(0, 3)) {
        try {
            const validation = await validateProxyUrl(scriptUrl);
            if (!validation.valid) {
                console.warn(`[Security] Blocked script fetch: ${validation.reason}`);
                continue;
            }
...
async function searchIframesRecursively(pageUrl, htmlData, referer, depth) {
    ...
    for (let iframeSrc of allIframes) {
        ...
        try {
            ...
            const validation = await validateProxyUrl(iframeSrc);
            if (!validation.valid) {
                console.warn(`[Security] Blocked iframe fetch: ${validation.reason}`);
                continue;
            }
