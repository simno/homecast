        ...
        if (result.statusCode === 200) {
            ...
            if (getPairingStore().isPaired(ip)) {
                ...
                try {
                    ...
                    // Retry /play after successful pair-verify
                    ...
                    if (retryResult.statusCode === 200) {
                        ...
                        return;
                    } else if (retryResult.statusCode === 403 || retryResult.statusCode === 401) {
                        ...
                        if (!res.headersSent) {
                            res.status(401).json({
                                ...
                                error: 'Pairing may be invalid. Please re-enter the PIN displayed on your Apple TV.',
                                detail: 'Pair-verify succeeded but /play was still rejected.'
                            });
                        }
                    } else {
                        if (!res.headersSent) {
                    ...
                    if (!res.headersSent) {
                        res.status(401).json({
                            needsPairing: true, deviceIp: ip,
                            deviceName: devices[ip]?.name || ip,
                            error: 'Pair-verify failed. Please re-enter the PIN on your Apple TV.',
                            detail: verifyErr.message
                        });
                    }
                }
            } else {
                // Not paired — require pairing
                console.log(`[AirPlay] Device ${ip} requires pairing (not paired)`);
                if (!res.headersSent) {
                    res.status(401).json({
                        needsPairing: true, deviceIp: ip,
                        deviceName: devices[ip]?.name || ip,
                        error: 'AirPlay device requires a PIN to pair. Enter the code displayed on your Apple TV.',
