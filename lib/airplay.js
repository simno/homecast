    ...
    browser.on('update', (data) => {
        ...
        if (deviceId) {
            ...
            if (existing && existing.ip !== ip) {
                const existingPreferred = !isPrivateIP(existing.ip);
                const newPreferred = !isPrivateIP(ip);
...
async function castToAirPlayDevice(ip, url, proxy, referer, quality, res, type) {
...
// Remote control over AirPlay 1: pause/play set the playback rate, seek
// skips `value` seconds from the current position. The Apple TV's own remote
// owns the volume.
async function controlAirPlayPlayback(ip, action, value) {
    if (!activeAirPlaySessions.has(ip)) throw new Error('No active session found for this device');

    let result;
    if (action === 'pause' || action === 'play') {
        result = await airPlayRequest(ip, `/rate?value=${action === 'pause' ? '0.000000' : '1.000000'}`, 'POST');
    } else if (action === 'seek' || action === 'seekTo') {
        const scrub = await airPlayRequest(ip, '/scrub', 'GET');
        const fields = Object.fromEntries(scrub.body.toString('utf8').split('\n')
            .map(line => line.split(':').map(part => part.trim()))
            .filter(([key, val]) => key && val !== undefined));
        const position = parseFloat(fields.position) || 0;
        const duration = parseFloat(fields.duration);
        let target = Math.max(0, action === 'seek' ? position + value : value);
        if (Number.isFinite(duration) && duration > 0) target = Math.min(target, Math.max(0, duration - 1));
        result = await airPlayRequest(ip, `/scrub?position=${target.toFixed(6)}`, 'POST');
    } else {
        throw new Error('Apple TV volume is set with its own remote');
    }
    if (result.statusCode >= 400) throw new Error(`Apple TV returned HTTP ${result.statusCode}`);
    // AirPlay 1 has no status events: tell the page what the rate change did.
    if (action === 'pause' || action === 'play') {
        broadcast({
            type: 'playerStatus', deviceIp: ip,
            status: { playerState: action === 'pause' ? 'PAUSED' : 'PLAYING' }, delay: 0
        });
    }
    return { volume: null };
}
