...
// Searches to run again when the user asks for a rescan (each discovery
// method registers its own).
const rescanHooks = [];
function onRescan(fn) {
    rescanHooks.push(fn);
}

function rescanDevices() {
    console.log('[Discovery] Rescan requested');
...
function registerDevice(ip, name, deviceId, source) {
    if (devices.has(ip) && devices.get(ip).type === 'airplay') return; // Don't overwrite AirPlay

    if (deviceId) {
        const existing = [...devices.values()].find(d => d.id === deviceId && d.ip !== ip);
        if (existing) {
            const newPreferred = !isPrivateIP(ip);
            const oldPreferred = !isPrivateIP(existing.ip);
    ...
    devices.set(ip, { name, ip, host: ip, id: deviceId, type: 'chromecast' });
    deviceLastSeen.set(ip, Date.now());
...
function initMDNSDiscovery() {
    ...
    browser.on('update', (data) => {
        ...
        if (isCast && data.addresses?.[0]) {
            ...
            const ip = data.addresses[0];
            if (devices.has(ip)) deviceLastSeen.set(ip, Date.now());
        }
    });
...
function initDiscovery() {
    ...
    if (IS_DEV) {
        try {
            ...
            devices.set(mockIp, {
                ...
                id: 'mock-chromecast-dev', type: 'chromecast', isMock: true, port: mockDevice.port
            });
    ...
    setInterval(() => {
        ...
        for (const [ip, device] of devices) {
            if (IS_DEV && device.isMock) continue;
            ...
            if (now - lastSeen > STALE_DEVICE_TIMEOUT_MS) {
                console.log(`[Discovery] Removing stale: ${device.name} (${ip})`);
                devices.delete(ip);
...
module.exports = { initDiscovery, onRescan, rescanDevices };
