...
function computeSRP(username, password, salt, serverPubKey) {
    ...
    // JS `%` can return a negative result for a negative dividend; B is in
    // [0, N-1] but the subtraction above isn't, so normalize back into range.
    const base = ((B_bn - kgx) % N_bn_val + N_bn_val) % N_bn_val;
    const exp = (a + u * x) % (N_bn_val - 1n);
    const S_bn = modPow(base, exp, N_bn_val);
...
async function pairWithDevice(ip, pin) {
    ...
    const deviceName = devices.get(ip)?.name || ip;
    try {
