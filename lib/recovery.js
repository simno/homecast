...
function initializeStreamRecovery(deviceIp, streamUrl, referer, quality, streamType) {
    streamRecovery.set(deviceIp, {
        ...
        // Carry the type the initial load resolved, so a recovery reload does
        // not re-declare a finished recording as LIVE and end it immediately.
...
async function attemptStreamRecovery(deviceIp, recovery) {
    try {
        ...
        const media = {
            contentId: `http://${localIp}:${PORT}/proxy?url=${encodeURIComponent(recovery.streamUrl)}&referer=${encodeURIComponent(recovery.referer || '')}&quality=${encodeURIComponent(recovery.quality || 'highest')}`,
            contentType: 'application/x-mpegURL',
            streamType: recovery.streamType || 'LIVE'
        };
