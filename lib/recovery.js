...
const { reloadOptions } = require('./subtitles');
const { MAX_RECOVERY_ATTEMPTS, STALL_TIMEOUT } = require('./utils');
...
// `media` is exactly what was loaded on the receiver ({ contentId,
// contentType, streamType }), so a recovery reload replays the same thing —
// proxied or not, HLS, DASH or MP4.
function initializeStreamRecovery(deviceIp, media) {
    streamRecovery.set(deviceIp, {
        ...
        // Includes the stream type the initial load resolved, so a recovery
        // reload does not re-declare a finished recording as LIVE and end it.
        media: { ...media }
    });
        ...
        if (bufferHealth && bufferHealth.lastState === 'BUFFERING' && bufferHealth.lastBufferStart) {
            ...
            if (bufferingDuration > STALL_TIMEOUT && timeSinceLastRequest > STALL_TIMEOUT) {
                if (!recovery.stallDetected && recovery.recoveryAttempts < MAX_RECOVERY_ATTEMPTS) {
                    ...
                    // Don't await: attemptStreamRecovery has its own try/catch
                    // and includes a 2s sleep, so awaiting it here would
                    // serialize recovery across every stalled device on this
                    // 10s tick instead of letting them run concurrently.
                    attemptStreamRecovery(deviceIp, recovery);
                }
            }
    ...
    try {
        ...
        await new Promise((resolve) => {
            try {
                ...
                // castv2's sessionRequest reads currentSession.mediaSessionId
                // synchronously, and a receiver that answers a health probe's
                // GET_STATUS with an empty status array leaves currentSession
                // undefined — so a session with no media becomes impossible to
                // stop. The stop is only housekeeping before the reload, and
                // abandoning the recovery here is what turns a recoverable
                // stall into a dead session.
                console.error('[Recovery] Stop failed:', e.message);
                resolve();
            }
        });
        ...
        player.load(media, { autoplay: true, ...reloadOptions(deviceIp, media) }, (err, _status) => {
            if (err) {
...
module.exports = {
    ...
    startStallDetection,
    checkStreamStalls // one detection tick, for tests
};
